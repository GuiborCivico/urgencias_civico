...
ppar_30 <- function(fuente,umbral_, model,model.name) {
  pr <- fuente
  ...
  data.frame(
    Modelo = model.name,
    Umbral = umbral_,
    Aprobacion.real = 1-mean(pr$PAR30),
    Aprobacion.final = mean(pr$riesgo < umbral_),
    Acuarcy = (table(pr$PAR30,pr$prediccion)[1]+
                 table(pr$PAR30,pr$prediccion)[4])/nrow(pr),
    Sensitivity =(table(pr$PAR30,pr$prediccion)[4]/
                    (table(pr$PAR30,pr$prediccion)[4]+
                       table(pr$PAR30,pr$prediccion)[2])),
    Specificity = (table(pr$PAR30,pr$prediccion)[1]/
                     (table(pr$PAR30,pr$prediccion)[1]+
                        table(pr$PAR30,pr$prediccion)[3])))
} # Evaluación PAR30
...
## 2.2. Modelo persona ----
m2 <- glm(PAR30~antiguedad+semana+
            # Persona
